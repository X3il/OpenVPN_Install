...
if readlink /proc/$$/exe | grep -q "dash"; then
	echo 'Данный инсталлятор должен быть запущен с "SH"'
	exit
fi
...
if [[ $(uname -r | cut -d "." -f 1) -eq 2 ]]; then
	echo "Система работает под управлением старого ядра, которое несовместимо с этим установщиком."
	exit
fi
...
Поддерживаемые дистрибутивы: Ubuntu, Debian, Alma Linux, Rocky Linux, CentOS и Fedora."
	exit
fi

if [[ "$os" == "ubuntu" && "$os_version" -lt 1804 ]]; then
	echo "Для использования этого установщика требуется Ubuntu 18.04 или более поздняя версия.
...
if [[ "$os" == "centos" && "$os_version" -lt 7 ]]; then
	echo "Для использования этого установщика требуется CentOS 7 или более поздняя версия.
...
if [[ "$EUID" -ne 0 ]]; then
	echo "Этот установщик должен быть запущен с правами суперпользователя."
	exit
fi

if [[ ! -e /dev/net/tun ]] || ! ( exec 7<>/dev/net/tun ) 2>/dev/null; then
	echo "В системе отсутствует устройство TUN.
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	if [[ $(ip -6 addr | grep -c 'inet6 [23]') -gt 1 ]]; then
		...
		echo
		echo "Какой IPV6 адрес должен использоваться?"
		ip -6 addr | grep 'inet6 [23]' | cut -d '/' -f 1 | grep -oE '([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}' | nl -s ') '
		read -p "IPv6 address [1]: " ip6_number
		until [[ -z "$ip6_number" || "$ip6_number" =~ ^[0-9]+$ && "$ip6_number" -le "$number_of_ip6" ]]; do
			echo "$ip6_number: Неправильный выбор."
			read -p "IPv6 address [1]: " ip6_number
		done
	...
	echo
	echo "Какой протокол должен использовать OPNEVPN?"
	echo "   1) UDP (Для потерянезависимого трафика)"
	echo "   2) TCP (Для потерязависимого трафика)"
	...
	echo
	echo "Какой порт будет слушать сервер OpenVPN?"
	...
	echo "   1) Те, что используются сейчас"
	echo "   2) Google"
	echo "   3) CloudFlare"
	echo "   4) OpenDNS"
	echo "   5) Quad9"
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo "Пожалуйста, настройте вручную фаерволл на основе того, что сами выбираете."
else
	clear
	...
	echo "   2) Отключить доступ клиенту"
	...
	case "$option" in
		1)
			...
			while [[ -z "$client" || -e /etc/openvpn/server/easy-rsa/pki/issued/"$client".crt ]]; do
				echo "$client: Неправильное имя клиента."
			...
			new_client
			echo
			echo "$client добавлен. Файл конфигурации доступен в:" ~/"$client.ovpn"
			exit
		;;
		2)
			...
			echo
			read -p "Подтвердите $client удаление? [y/N]: " revoke
			until [[ "$revoke" =~ ^[yYnN]*$ ]]; do
				echo "$revoke: invalid selection."
				read -p "Подтвердить $client деактивацию? [y/N]: " revoke
			done
			if [[ "$revoke" =~ ^[yY]$ ]]; then
				cd /etc/openvpn/server/easy-rsa/
				...
				chown nobody:"$group_name" /etc/openvpn/server/crl.pem
				echo
				echo "$client отключен!"
			else
				echo
				echo "$client отключение отменено!"
			fi
			exit
		;;
		3)
